...
public class GadgetScript : MonoBehaviour
{
    ...
    void OnLeftShoulder()
    {
        if (gadgetCDcounter > 0 || FindAnyObjectByType<Global>().grappling || FindAnyObjectByType<Global>().zipping|| FindAnyObjectByType<Global>().inmenu_inv_shop || playerjump.stuckinwall)
        {
            return;
        }
        UseGadget();
